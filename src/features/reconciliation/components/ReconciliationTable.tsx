...
export function ReconciliationTable({ evaluatedOpportunities, onOpportunityMapped }: ReconciliationTableProps) {
  ...
  const [selectedCandidates, setSelectedCandidates] = useState<Record<string, Set<number>>>({});
  ...
  const toggleCandidate = (oppId: string, idx: number) => {
    setSelectedCandidates(prev => {
      const currentSet = prev[oppId] ? new Set(prev[oppId]) : new Set<number>();
      if (currentSet.has(idx)) {
        currentSet.delete(idx);
      } else {
        currentSet.add(idx);
      }
      return { ...prev, [oppId]: currentSet };
    });
  };

  const getSelectedTotal = (oppId: string, topCandidates: ExcelRowData[]) => {
    const set = selectedCandidates[oppId];
  ...
  const handleVerify = (opportunityId: string, topCandidates: ExcelRowData[]) => {
    const set = selectedCandidates[opportunityId];
    if (!set || set.size === 0) return;
    const selectedRows = Array.from(set).map(idx => topCandidates[idx]).filter(Boolean);
    const invoices = selectedRows.map(r => String(r.Invoice_number || r.Invoice || "")).filter(Boolean);
    
    handleAction(opportunityId, ReconciliationStatus.VERIFIED, {
      pr_invoice_number: invoices.join(','),
      pr_verified_data: selectedRows
    });
  };
    ...
    <div className="border rounded-xl bg-card text-card-foreground shadow-sm overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full text-sm text-left">
          ...
          <tbody>
            {evaluatedOpportunities.map(({ opportunity, matchColor, topCandidates, matchReasons }) => {
              const selectedTotal = getSelectedTotal(opportunity.id, topCandidates);
              const hasSelection = (selectedCandidates[opportunity.id]?.size || 0) > 0;
                    ...
                    <tr className="bg-muted/5 border-b border-border">
                      <td colSpan={5} className="p-0">
                        <div className="p-6">
                              ...
                              <ul className="space-y-2 mt-1 text-sm text-muted-foreground">
                                {matchReasons.map((r, i) => (
                                  <li key={i} className="flex items-start gap-2">
                                    <svg className={`w-4 h-4 shrink-0 mt-0.5 ${r.includes("CONFLICT") || r.includes("mismatch") ? "text-red-500" : "text-green-500"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          ...
                          {/* Data Lake Candidates List */}
                          <div className="mb-6">
                            <h4 className="font-semibold text-xs mb-3 uppercase text-muted-foreground tracking-wider">Top Candidates</h4>
                                ...
                                <table className="w-full text-xs text-left">
                                  ...
                                  <tbody>
                                    {topCandidates.map((candidate, idx) => {
                                      const isSelected = selectedCandidates[opportunity.id]?.has(idx) || false;
                                        ...
                                        >
                                          <td className="px-4 py-3">
                                            <input 
                                              ...
                                              onClick={(e) => e.stopPropagation()} // in case they click exactly the box
                          ...
                          <div className="flex flex-col sm:flex-row items-end justify-between gap-4 p-5 bg-background border rounded-lg shadow-sm">
                            ...
                            <div className="flex items-center gap-4 shrink-0 mt-4 sm:mt-0">
                              ...
                              <button
                                disabled={isPending || !hasSelection || matchColor === 'RED'}
                                onClick={() => handleVerify(opportunity.id, topCandidates)}
